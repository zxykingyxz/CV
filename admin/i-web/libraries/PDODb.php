...

{
     ...
     * @param string|array|object $_TYPE
    ...
    public function __construct($_TYPE, $host = null, $username = null, $password = null, $dbname = null, $port = null, $charset = null)

    {

        if (is_array($_TYPE)) { // if params were passed as array

            $this->connectionParams = $_TYPE;
        } elseif (is_object($_TYPE)) { // if type is set as pdo object
            ...
            switch (strtolower($operator)) {
                ...
                case 'in':
                    ...
                    if (is_object($val)) {
                        ...
                        $_COMparison .= $this->buildPair("", $val);
                    } else {
...

    {
        ...
        $_TYPEs = ["s" => "second", "m" => "minute", "h" => "hour", "d" => "day", "M" => "month", "Y" => "year"];
        ...
        if ($diff && preg_match('/([+-]?) ?([0-9]+) ?([a-zA-Z]?)/', $diff, $matches)) {
            ...
            if (!in_array($_TYPE, array_keys($_TYPEs))) {

                die($this->sendException('Generated Interval', '***', "invalid interval type in '{$diff}'", '***'));
            }

            $func .= " " . $incr . " interval " . $items . " " . $_TYPEs[$_TYPE] . " ";
        }
    ...
    public function createTable()
    {
        ...
        for ($i = 0; $i < count($_LANGAttr); $i++) {

            $table = $_LANGAttr[$i]["name"];

            $this->query = "CREATE TABLE $this->prefix$table (";

            for ($j = 0; $j < count($_LANGAttr[$i]['value']); $j++) {

                $column = $_LANGAttr[$i]['value'][$j]['col'];

                $_TYPE = $_LANGAttr[$i]['value'][$j]['type'];

                $sql .= "$column $_TYPE COLLATE utf8_unicode_ci NOT NULL,";
            }
    ...
    private function sendException($name, $errorNumber = '', $erroMessage = '', $sql = '')
    {
        ...
        ob_start();
        include _lib . "/sample/pdo/exception.php";
    ...
    public function createlogs($user = 'Unnamed', $_ACT = null, $_COM = null, $id = null)
    {
        global $https_config;
        ...
        fwrite($file, $ip_user . "(" . $user . ")" . "--" . $_ACT . "--" . $_COM . "(" . $id . ") (" . $time_act . ")\n");
        fclose($file);
