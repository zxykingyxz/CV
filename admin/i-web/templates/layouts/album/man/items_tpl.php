                ...
                $.confirm({
                    ...
                    buttons: {
                        success: {
                            ...
                            btnClass: 'btn-blue',
                            action: function() {
                                redirect("index.html?com=album&act=delete&type=<?= $_GET['type'] ?>&curPage=<?= $_GET['curPage'] ?>&listid=" + listid);
                            }
                        },
                        cancel: {
                            text: 'Hủy ngay!',
...
<div class="control_frm">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
            <li><a
                    href="index.html?com=album&act=man<?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>"><span><?= $GLOBAL[$_COM][$_TYPE]['title'] ?></span></a>
            </li>
            <?php if ($_GET['keyword'] != '') { ?>
        ...
        <div class="box-admin" style="display:flex; align-items:center;">
            <div class="box-action">
                <a class="btn btn-sm bg-gradient-primary text-white"
                    href="index.html?com=album&act=add<?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>">
            ...
            <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
                <thead>
                    <tr>
                        ...
                        <td class="sortCol">
                            <div>Tiêu đề <?= $GLOBAL[$_COM][$_TYPE]['title_main'] ?><span></span></div>
                        </td>
                    ...
                    <?php for ($i = 0, $count = count($items); $i < $count; $i++) { ?>
                        <tr>
                            ...
                            <td class="title_name_data">
                                <a href="index.html?com=album&act=edit&id_list=<?= $items[$i]['id_list'] ?>&id_cat=<?= $items[$i]['id_cat'] ?>&id_item=<?= $items[$i]['id_item'] ?>&id_sub=<?= $items[$i]['id_sub'] ?>&id=<?= $items[$i]['id'] ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>"
                                    class="tipS SC_bold">
                            ...
                            <td align="center">
                                <label class="stardust-checkbox checkOnOff">
                                    <input class="stardust-checkbox__input" data-id="<?= $items[$i]['id'] ?>"
                                        data-table="table_album" data-type="noibat" rel="<?= $items[$i]['noibat'] ?>"
                                        <?php if ($items[$i]['noibat'] == 1) echo 'checked'; ?> name="onOff" type="checkbox"
                                        style="display:none">
                                    <div class="stardust-checkbox__box"></div>
                                </label>
                            </td>
                            <td align="center">
                                <label class="stardust-checkbox checkOnOff">
                                    <input class="stardust-checkbox__input" data-id="<?= $items[$i]['id'] ?>"
                                        data-table="table_album" data-type="hienthi" rel="<?= $items[$i]['hienthi'] ?>"
                            ...
                            <td class="actBtns">
                                <a class="text-primary"
                                    href="index.html?com=album&act=edit&id_list=<?= $items[$i]['id_list'] ?>&id_cat=<?= $items[$i]['id_cat'] ?>&id_item=<?= $items[$i]['id_item'] ?>&id_sub=<?= $items[$i]['id_sub'] ?>&id=<?= $items[$i]['id'] ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>"
