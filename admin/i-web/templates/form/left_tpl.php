...
$class_option_left_menu = "gap-2  pl-3 py-2 text-xs [&.active]:text-[var(--html-all-admin)]";
...
<div class="h-[100vh] bg-[var(--html-bg-all-admin)] w-[220px] ">
    <div class="w-full h-full flex flex-wrap flex-col px">
        <div class="w-full px-5 py-3 border-b border-gray-400 relative ">
            <div class="w-[70%]">
                <?= $func->addHrefImg([
                    'addhref' => true,
        ...
        <div class="flex-1 overflow-x-hidden overflow-y-auto scroll-design-one max-h-[initial] px-2 pt-2">
            <ul class="form_menu_admin  ">
                <?php
                // ----------------- tác giả -------------------
                ...
                <li class=" border-b bg-inherit <?= $border_color_left_menu ?>">
                    <div class="btn_menu_admin group peer rounded  flex  w-full <?= $text_color_left_menu ?> <?= $background_left_menu ?>  <?= ($check_author) ? "on active" : "" ?> " data-nb="author_admin">
                    ...
                    <div class="data_menu_admin hidden " style="<?= ($check_author) ? "display: block;" : "" ?>" data-nb="author_admin">
                        <ul class=" w-full  <?= $text_color_left_menu ?> bg-inherit  transition-all duration-300">
                            <li class="pl-3">
                                <a class="flex items-center <?= $class_option_left_menu ?> <?= ($check_items_author) ? "active" : "" ?> transition-all duration-300" href="<?= $link_author ?>" title="<?= $GLOBAL['baiviet'][$_TYPE_author]['title'] ?>">
                                    <i class=" fas fa-angle-right text-sm font-light leading-none"></i>
