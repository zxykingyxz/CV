<div class="sticky top-0 left-0 w-full z-20">
    <div class="w-full bg-gray-100 shadow-md shadow-gray-300 py-2 px-2  sm:px-3">
        <div class="w-full flex items-center justify-between gap-2">
            <div class="flex-initial inline-flex gap-3 justify-center items-center">
                ...
                <div class="inline-flex gap-2 justify-center items-center">
                    <div class="w-[28px] aspect-[1/1] flex justify-center items-center text-gray-500 border border-gray-500 rounded-full">
                        <i class=" fas fa-user text-[13px]"></i>
                    </div>
                    <div class="text-[13px] text-gray-500 hidden lg:inline-block">
                        <span> <?= "Xin chào, " . $dataAcconutLogged->username . " !" ?> </span>
                    </div>
                </div>
            </div>
            <div class="flex-1 flex justify-end justify-items-end items-center gap-6">
                <a href="<?= $func->getUrlParam(["com" => "cache", "act" => "delete"]); ?>" title="Xóa Cache" target="_blank" class="inline-flex justify-center items-center gap-2 text-gray-500">
                    <div class="">
                        <i class="fas fa-folder-minus text-[16px] "></i>
                    </div>
                    <span class="text-[11px] hidden lg:inline-block">Xóa Cache </span>
                </a>
                <a href="<?= $https_config . "sitemap.xml" ?>" title="Cập Nhật Sitemap" target="_blank" class="inline-flex justify-center items-center gap-2 text-gray-500">
                    <div class="">
                        <i class="far fa-sitemap text-[14px] "></i>
                    </div>
                    <span class="text-[11px] hidden lg:inline-block">Cập Nhật Sitemap </span>
                </a>
                <a href="<?= $https_config ?>" title="Vào Website" target="_blank" class="inline-flex justify-center items-center gap-2 text-gray-500">
                    <div class="">
                        <i class="fas fa-share-square text-[14px] "></i>
                    </div>
                    <span class="text-[11px] hidden lg:inline-block">Vào Website</span>
                </a>
                <a href="<?= $func->getUrlParam(["com" => "user", "act" => "edit", "type" => "user", "id" => $dataAcconutLogged->id]); ?>" title="Thông tin tài khoản" class="inline-flex justify-center items-center gap-2 text-gray-500">
                    <div class="">
                ...
                <div class="group relative inline-flex justify-center items-center cursor-pointer z-10">
                    <i class="<?= ($tong_count > 0) ? "shake_design" : "" ?> far fa-bell text-gray-500 text-[20px]"></i>
                    ...
                    <div class=" group-hover:visible group-hover:opacity-100 group-hover:scale-y-100 opacity-0 invisible scale-y-75 absolute top-full right-[-10px] w-[200px]  transition-all duration-300 ">
                        ...
                        <div class="bg-white overflow-hidden rounded shadow-md shadow-gray-300 w-full text-sm font-normal flex flex-wrap">
                            <?php
                            // ----------------- Liên hệ -------------------
                            $_TYPE_booking = 'booking';
                            $data_booking = $GLOBAL[$_TYPE_booking];
                            if (!empty($data_booking)) {
                            ?>
                                <?php foreach ($data_booking as $key_booking => $value_booking) {
                                    $check_items_booking = $func->checkLeftMenu(["com" => $_TYPE_booking, "act" =>  $check_act, "type" => $key_booking]);
                                    $link_booking = $func->getUrlParam(["com" => $_TYPE_booking, "act" =>  "man", "type" => $key_booking]);
                ...
                <a href="<?= $func->getUrlParam(["com" => "user", "src" => "user", "act" => "logout"]); ?>" title="Logout" class="inline-flex justify-center items-center gap-2 text-gray-500">
                    <div class="">
