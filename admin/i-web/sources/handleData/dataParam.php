<?php
$_COM = (isset($_GET['com'])) ? htmlspecialchars($_GET['com']) : "";
...
$array_param_data = ["keywords", "status", "month", "year", "page"];
...
if (!empty($array_param_data)) {
    foreach ($array_param_data as $value_param) {
        switch ($value_param) {
            case "year":
                $value_check = (isset($_REQUEST[$value_param])) ? $func->getDataNumber(addslashes($_REQUEST[$value_param])) : date("Y", time());
                break;
            case "month":
                $value_check = (isset($_REQUEST[$value_param])) ? $func->getDataNumber(addslashes($_REQUEST[$value_param])) : date("m", time());
                break;
            default:
