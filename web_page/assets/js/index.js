_FRAMEWORK = {
    ...
    thumbsGallery: function () {
        ...
        if ($("body .button_images_thumbs_gallery").length > 0) {
            thumbsSlider = new Swiper(".button_images_thumbs_gallery", {
        ...
        // Main slider
        new Swiper(".view_images_thumbs_gallery", {
            spaceBetween: 10,
            navigation: {
        ...
        if ($("body .button_images_thumbs_gallery_project").length > 0) {
            thumbsSliderProject = new Swiper(".button_images_thumbs_gallery_project", {
                slidesPerView: 4, // Số thumbnails hiển thị cùng lúc
                spaceBetween: 8, // Khoảng cách giữa các thumbnails
                watchSlidesProgress: true, // Theo dõi trạng thái thumbnail
            });
        ...
        // Main slider
        new Swiper(".view_images_thumbs_gallery_project", {
            spaceBetween: 10,
            navigation: {
                nextEl: ".button_next_images_thumbs_gallery_project", // Nút chuyển tiếp
                prevEl: ".button_prev_images_thumbs_gallery_project", // Nút quay lại
            },
            thumbs: {
                swiper: thumbsSliderProject, // Kết nối với thumbs slider
            },
        });
    },
    zoomDetail: function () {
    ...
    Menu: function () {
        ...
        $("body").on("click", select_design1.btn_dm, function (event) {
            ...
            if (_this.closest(select_design1.items).hasClass("active")) {
                ...
                if (data_level == 0) {
                    _this
                        .closest(select_design1.form_menu)
                        .find(select_design1.items + "[data-level='" + data_level + "']")
                        .show();
                } else {
                    _this
                        .closest(select_design1.form_menu)
                        .find(select_design1.items + "[data-type='" + data_type + "' ][data-level='" + (data_level - 1) + "'][data-after='" + data_before + "']")
                        .addClass("active")
                        .show();
                    _this
                        .closest(select_design1.form_menu)
                        .find(select_design1.items + "[data-type='" + data_type + "' ][data-level='" + data_level + "'][data-before='" + data_before + "']")
                        .show();
                }
            } else {
                // mở
                ...
                _this
                    .closest(select_design1.form_menu)
                    .find(select_design1.items + "[data-type='" + data_type + "' ][data-level='" + (data_level + 1) + "'][data-before='" + data_after + "']")
                    .show();
            }
        });
    },
