...
function log(str) {
    console.log(`%c${str}`, `font-size:18px;color:orange`);
}
...
function getUrlParam(paramsToAdd = {}, dataParamClose = [], baseUrl = window.location.origin + window.location.pathname) {
    ...
    for (const [key, value] of urlParams.entries()) {
        if (!dataParamClose.includes(key)) {
            params[key] = value ? value.trim() : ""; // Tránh lỗi nếu value là null hoặc undefined
        }
    }
    ...
    // Tạo URL mới (giữ nguyên đường dẫn nếu không có ?)
