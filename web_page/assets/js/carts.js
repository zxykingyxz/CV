...
_CART.deleteCart = () => {
    if (exists("body " + selector.delete_all_product)) {
        $("body").on("click", selector.delete_all_product, function () {
            var code = $('input[name="check-product"]:checked')
                .map(function () {
                    return this.value;
                })
                .get()
            ...
            if (code == "") {
                ...
                _CART.showConfirm({
                    title: lang.xac_nhan,
                    message: lang.ban_co_chac_muon_xoa_muc_nay,
                    confirmText: lang.dong_y,
                    cancelText: lang.huy_ngay,
                    onConfirm: function () {
    ...
    if (exists(".delCart")) {
        $("body").on("click", ".delCart", function () {
            ...
            _CART.showConfirm({
                title: lang.xac_nhan,
                message: lang.ban_muon_xoa_san_pham,
                confirmText: lang.dong_y,
                cancelText: lang.huy_ngay,
                onConfirm: function () {
                    funcDeleteCart(code, "deleteCart");
...
_CART.updateCart = () => {
    ...
    $("body").on("click", selector.button_qty_items_cart_product, function () {
        ...
        qty_tmp = $(this)
            .closest(selector.items_cart_product)
            .find("input" + selector.number_cart_product)
            .val();
        $this = $(this);
        switch (macth) {
            case 1:
    ...
    $("body").on("change", selector.select_attribute, function () {
        let _this = $(this);
        id = _this.val();
        id = _this.val();
        qty = _this
        ...
        code = _this.closest(selector.items_cart_product).data("code");
        pid = _this.closest(selector.items_cart_product).data("id-product");
        var params = {
            code: code,
            pid: pid,
            qty: qty,
...
_CART.clickAttribute = () => {
    $("body").on("click", selector.btn_attribute, function () {
        let _this = $(this);
        let data_id = _this.data("id-attribute");
        let data_form = _this.data("form-attribute");
        let data_type = _this.data("type");
        let data_id_product = _this.data("id-product");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 200) {
                    $("body " + selector.btn_attribute + "[data-type=" + data_type + "]").removeClass("active");
...
_CART.showNotification = function (
    data = {
        title: null,
        message: null,
        status: "success",
