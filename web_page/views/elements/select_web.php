...
$class_text = " px-3 h-[48px] text-sm font-normal font-main-400 ";
...
<?php if ($form !== false) { ?>
    <label class=" <?= $class_form ?>">
        <?php if ($no_label !== true) { ?>
            ...
            <?php } ?>
        <?php } ?>
        <div class="bg-white border shadow-sm border-slate-300 placeholder-slate-400 has-[:disabled]:bg-slate-50 has-[:disabled]:text-slate-500 has-[:disabled]:border-slate-200 has-[:focus]:outline-none has-[:focus]:border-[var(--html-bg-website)] has-[:focus]:ring-[var(--html-bg-website)] block  has-[:focus]:ring-1  has-[:disabled]:shadow-none overflow-hidden  <?= $class_form_seclect ?>">
