...
<div class="wrapper">
    <div class="control_frm">
        <div class="bc">
            <ul id="breadcrumbs" class="breadcrumbs">
                <li><a href="index.html?com=baiviet&act=add_item&tbl=<?= $_GET['tbl'] ?><?php if ($_REQUEST['id'] != '') echo '&id=' . $_REQUEST['id']; ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>"><span><?= $GLOBAL_LEVEL3[$com][$type]['title'] ?></span></a>
                </li>
    ...
    <form name="supplier" id="validate" class="form form-validate" action="index.html?com=baiviet&act=save_item&tbl=<?= $_GET['tbl'] ?><?php if ($_REQUEST['id'] != '') echo '&id=' . $_REQUEST['id']; ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>" method="post" enctype="multipart/form-data" autocomplete="off" accept-charset="utf-8">
        <div class="mtop0">
                                ...
                                <div class="formRight">
                                    <div class="box-alias" style="position: relative;display: -ms-flexbox;display: flex;-ms-flex-wrap: wrap;flex-wrap: wrap;-ms-flex-align: stretch;align-items: stretch;width: 100%;">
                                        ...
                                        <div style="position: relative;-ms-flex: 1 1 auto;flex: 1 1 auto;width: 1%;margin-bottom: 0;">
                                            <input readonly data-validation="required" data-validation-error-msg="Url không được để trống" type="text" name="data[tenkhongdau_<?= $k ?>]" title="Nhập tên không dấu" id="alias_<?= $k ?>" class="tipS input100" value="<?= @$item['tenkhongdau_' . $k] ?>" />
                                        </div>
                    ...
                    <?php foreach ($config['lang'] as $k => $v) { ?>
                        <?php if ($GLOBAL_LEVEL3[$com][$type]['mota'] == true) { ?>
                            <div class="formRow lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>">
                                <label><?= _mota ?> [<?= $v ?>]</label>
                                <div class="formRight">
                                    <div <?php if ($GLOBAL_LEVEL3['baiviet']['san-pham']['mota-ckeditor'] == true) echo 'class="ck_editor"'; ?>>
                                        <textarea title="Nhập mô tả . " data-height="400" id="mota_<?= $k ?>" <?= ($GLOBAL_LEVEL3[$com][$type]['mota-ckeditor'] == true) ? 'class="ck_editors"' : 'rows="8"' ?> name="data[mota_<?= $k ?>]"><?= htmlspecialchars_decode($item['mota_' . $k]) ?></textarea>
                                    </div>
                                    <div class="clear"></div>
                                </div>
                            </div>
                        ...
                        <?php if ($GLOBAL_LEVEL3[$com][$type]['noidung'] == true) { ?>
                            <div class="formRow lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>">
                                ...
                                <div class="formRight">
                                    <div <?php if ($GLOBAL_LEVEL3['baiviet']['san-pham']['noidung-ckeditor'] == true) echo 'class="ck_editor"'; ?>>
                                        <textarea title="Nhập mô tả . " data-height="400" id="noidung_<?= $k ?>" <?= ($GLOBAL_LEVEL3[$com][$type]['noidung-ckeditor'] == true) ? 'class="ck_editors"' : 'rows="8"' ?> name="data[noidung_<?= $k ?>]"><?= htmlspecialchars_decode($item['noidung_' . $k]) ?></textarea>
                                    </div>
                                    <div class="clear"></div>
                                </div>
                            </div>
            ...
            <div class="<?= ($GLOBAL_LEVEL3[$com][$type]['full'] == true) ? 'oneOne' : 'colRight' ?>">
                <div class="widget mtop0">
                    ...
                    <div class="formRow">
                        <div class="formRight">
                            ...
                            <input type="text" class="tipS" value="<?= isset($item['stt']) ? $item['stt'] : $func->checkNumb('stt', $com . '_item', $type) ?>" name="data[stt]" style="width:40px; text-align:center;" onkeypress="return OnlyNumber(event)" original-title="Số thứ tự của danh mục, chỉ nhập số">
                        </div>
                        <div class="clear"></div>
                    </div>
                ...
                */ ?>
                                <div class="formRow lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>">
                                    <div class="formRight">
                                        <input readonly="readonly" type="text" style="width:45px; margin-top:10px; text-align:center;" name="des_char" id="des_char" value="<?= @$item['des_char'] ?>" /> <?= _kytu ?> <b>(Từ
                                            160-300
                                            ký tự)</b>
                                    </div>
                                    <div class="clear"></div>
