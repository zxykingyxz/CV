<nav class="sidebar-nav sidebar-bunker">
    ...
    <ul class="metismenu" id="menu1">
        ...
        <?php if (isset($GLOBAL['album']) && count($GLOBAL['album']) > 0) { ?>
            ...
            <?php foreach ($GLOBAL['album'] as $key => $value) { ?>
                ...
                <li <?php if (isset($kiemtra)) {
                    ...
                    } ?> class="<?= ($com == 'album' && $key == $_GET['type']) ? "active" : '' ?>">

                    <a class="has-arrow" href="#" aria-expanded="false"><i class="nav-icon text-sm fal fa-images"></i><?= $value['title_main'] ?></a>

                    <ul aria-expanded="<?= ($com == 'album' && $key == $_GET['type']) ? "true" : 'false' ?>" class="collapse <?= ($com == 'album' && $key == $_GET['type']) ? "in" : '' ?>" <?= ($com == 'album' && $key == $_GET['type']) ? "" : 'style="height: 0px;"' ?>>

                        <?php if ($GLOBAL['album'][$key]['list'] == true) { ?>

                            <li <?php if (isset($kiemtra)) {
                                    if ($func->checkPermissions('album', 'man_list', $key)) echo 'class="none"';
                        ...
                        <li <?php if (isset($kiemtra)) {
                                if ($func->checkPermissions('album', 'man', $key)) echo 'class="none"';
