<div class="control_frm">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
            <li><a href="index.html?com=photo&act=man<?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?>"><span><?= $GLOBAL[$com][$type]['title'] ?></span></a>
            </li>
                        ...
                        <?php for ($i = 0, $count = count($items); $i < $count; $i++) { ?>
                            <tr>
                                ...
                                <td class="title_name_data t-center">
                                    <a href="index.html?com=photo&act=edit&id=<?= $items[$i]['id'] ?><?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?><?php if ($_GET['page'] != '') echo '&page=' . $_GET['page']; ?>" class="tipS SC_bold"><?= $items[$i]['ten_vi'] ?></a>
                                </td>
                                <?php if ($GLOBAL[$com][$type]['phone'] == true) { ?>
                                    <td class="title_name_data t-center">
                                        <a href="index.html?com=photo&act=edit&id=<?= $items[$i]['id'] ?><?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?><?php if ($_GET['page'] != '') echo '&page=' . $_GET['page']; ?>" class="tipS SC_bold"><?= $items[$i]['phone'] ?></a>
                                    </td>
                                <?php } ?>
                                <?php if ($GLOBAL[$com][$type]['email'] == true) { ?>
                                    <td class="title_name_data t-center">
                                        <a href="index.html?com=photo&act=edit&id=<?= $items[$i]['id'] ?><?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?><?php if ($_GET['page'] != '') echo '&page=' . $_GET['page']; ?>" class="tipS SC_bold"><?= $items[$i]['email'] ?></a>
                                    </td>
                                <?php } ?>
                                <?php if ($GLOBAL[$com][$type]['zalo'] == true) { ?>
                                    <td class="title_name_data t-center">
                                        <a href="index.html?com=photo&act=edit&id=<?= $items[$i]['id'] ?><?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?><?php if ($_GET['page'] != '') echo '&page=' . $_GET['page']; ?>" class="tipS SC_bold"><?= $items[$i]['zalo'] ?></a>
                                    </td>
                                <?php } ?>
                                <?php if ($GLOBAL[$com][$type]['address'] == true) { ?>
                                    <td class="title_name_data t-center">
                                        <a href="index.html?com=photo&act=edit&id=<?= $items[$i]['id'] ?><?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?><?php if ($_GET['page'] != '') echo '&page=' . $_GET['page']; ?>" class="tipS SC_bold"><?= $items[$i]['address'] ?></a>
                                    </td>
                                <?php } ?>
                                <?php foreach ($GLOBAL[$com][$type]['check'] as $key => $value) { ?>
                                ...
                                <td class="actBtns">
                                    <?php if (($kiemtra == 1 & $func->checkPermissions('photo', 'edit', $type)) || $kiemtra != 1) { ?>
                                        <a class="text-primary" href="index.html?com=photo&act=edit&id=<?= $items[$i]['id'] ?><?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?><?php if ($_GET['page'] != '') echo '&page=' . $_GET['page']; ?>" title="" class="smallButton tipS" original-title="Sửa hình ảnh"><i class="fas fa-edit"></i></a>
                                    <?php } ?>
                                    <?php if (($kiemtra == 1 & $func->checkPermissions('photo', 'delete', $type)) || $kiemtra != 1) { ?>
