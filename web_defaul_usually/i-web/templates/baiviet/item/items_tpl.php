...
<div class="control_frm">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
            <li><a href="index.html?com=baiviet&act=man_item&tbl=<?= $_GET['tbl'] ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>"><span><?= $GLOBAL_LEVEL3[$com][$type]['title'] ?></span></a>
            </li>
            <?php if ($_GET['keyword'] != '') { ?>
    ...
    <div class="oneOne">
        <div class="box-admin d-flex d-block-m align-items-center">
            <div class="box-action">
                <a class="btn btn-sm bg-gradient-primary text-white" href="index.html?com=baiviet&act=add_item&tbl=<?= $_GET['tbl'] ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>">
                        ...
                        <?php for ($i = 0, $count = count($items); $i < $count; $i++) { ?>
                            <tr>
                                ...
                                <?php if ($GLOBAL_LEVEL1[$com][$type]['img'] == true) { ?>
                                    <td align="center">
                                        <a href="index.html?com=baiviet&act=edit_item&tbl=<?= $_GET['tbl'] ?>&id=<?= $items[$i]['id'] ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?><?php if ($_REQUEST['page'] != '') echo '&page=' . $_REQUEST['page']; ?>" class="tipS SC_bold">
                                ...
                                <td class="title_name_data">
                                    <a href="index.html?com=baiviet&act=edit_item&tbl=<?= $_GET['tbl'] ?>&id_list=<?= $items[$i]['id_list'] ?>&id_cat=<?= $items[$i]['id_cat'] ?>&id=<?= $items[$i]['id'] ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>" class="tipS SC_bold"><?= $items[$i]['ten_vi'] ?></a>
                                </td>

                                <?php foreach ($GLOBAL_LEVEL3[$com][$type]['check_item'] as $key => $value) { ?>
                                    <td align="center">
                                        <label class="stardust-checkbox checkOnOff">
                                            <input class="stardust-checkbox__input" data-id="<?= $items[$i]['id'] ?>" data-table="table_baiviet_item" data-type="<?= $key ?>" rel="<?= $items[$i][$key] ?>" <?php if ($items[$i][$key] == 1) echo 'checked'; ?> name="onOff" type="checkbox" style="display:none">
                                            <div class="stardust-checkbox__box"></div>
                                        </label>
                                    </td>
                                <?php } ?>
                                ...
                                <td class="actBtns">
                                    <a class="text-primary" href="index.html?com=baiviet&act=edit_item&tbl=<?= $_GET['tbl'] ?>&id_list=<?= $items[$i]['id_list'] ?>&id_cat=<?= $items[$i]['id_cat'] ?>&id=<?= $items[$i]['id'] ?><?php if ($_REQUEST['type'] != '') echo '&type=' . $_REQUEST['type']; ?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><i class="fas fa-edit"></i></a>

                                    <a class="text-danger" data-product="<?= $items[$i]['id'] ?>" data-com="<?= $_GET['com'] ?>" data-act="delete_<?= $_GET['tbl'] ?>" data-tbl="<?= $_GET['tbl'] ?>" data-type="<?= $_GET['type'] ?>" data-page="<?= $_GET['page'] ?>" href="javascript:" data-js-delete title="" class="smallButton tipS" original-title="Xóa "><i class="fas fa-trash-alt"></i></a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
