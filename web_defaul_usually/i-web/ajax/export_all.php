...
$items_order = $db->rawQuery("SELECT * from #_order");
...
foreach ($items_order as $k => $v) {
	...
	$items_detail = $db->rawQuery("select * from #_order_detail where id_order='" . $id . "' order by id desc");
	...
	$status_order = $config['order-status'][$v['order_status']]['name'];
