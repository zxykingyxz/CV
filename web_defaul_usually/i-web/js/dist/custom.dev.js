...
function holdonOpen() {
  var theme = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "sk-rect";
  var text = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "Text here";
  var backgroundColor = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "rgba(0,0,0,0.8)";
  var textColor = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "white";
  var options = {
    theme: theme,
    message: text,
...
$(function () {
  ...
  $("#validate1").validationEngine(); //===== WYSIWYG editor =====//
  // holdon
  ...
  $.configureBoxes(); //===== Left navigation styling =====//
  ...
  $(document).bind('click', function (e) {
    var $clicked = $(e.target);
    if (!$clicked.parents().hasClass("ddnew")) $(".userMessage").slideUp(200);
  ...
  $('.chontatca input:checkbox').click(function () {
    var checkedStatus = this.checked;
    $(this).closest('.box-phanquyen').find('.body-check').find('input').each(function () {
      this.checked = checkedStatus;
  ...
  $('.body-check input:checkbox').change(function () {
    var checkedStatus = this.checked;

    if (checkedStatus == this.checked) {
      $(this).closest('.box-phanquyen').find('.chontatca').find('.checker > span').addClass('checked');
    } else {
      $(this).closest('.box-phanquyen').find('.chontatca').find('.checker > span').removeClass('checked');
    }
  }); //===== Sortable columns =====//

  $("table").tablesorter(); //===== Dynamic data table =====//
  ...
  $("#maxRangeAmount").val($(".uiMaxRange").slider("value")); //===== Form elements styling =====//
