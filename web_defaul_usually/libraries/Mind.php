...
class Mind
{
    public $session_path    =   null;
    public  $post           =   [];
    public  $base_url;
    public  $page_uri       =   'https://i-web.vn/';
    public  $page_current   =   '';
    public  $page_back      =   'https://i-web.vn/';
    public  $project_path   =   '';
    ...
    private $conf           =   [];
    private $db             =   [
        'type'     =>  'mysql', // mysql, sqlite, sqlsrv
    ...
    public function __construct($conf = array()){
        ...
        $this->conf = (isset($conf)) ? $conf : ['database'=>$this->db];
        
        /* Database connection */
        $dbStatus = true;
        if(isset($conf['database'])){
            $dbStatus = (($conf['database'] == '') OR (is_array($conf['database']) AND empty($conf['database']))) ? false : true;
        }
        if($dbStatus){
            $this->dbConnect($conf);
        }
    ...
    public function dbConnect($conf = array()){
        ...
        if(isset($conf['database']['type'])){ $this->db['type'] = $conf['database']['type'];}
        if(isset($conf['database']['host'])){ $this->db['host'] = $conf['database']['host'];}
        if(isset($conf['database']['dbname'])){ $this->db['dbname'] = $conf['database']['dbname'];}
        if(isset($conf['database']['username'])){ $this->db['username'] = $conf['database']['username'];}
        if(isset($conf['database']['password'])){ $this->db['password'] = $conf['database']['password'];}     
        if(isset($conf['database']['charset'])){ $this->db['charset'] = $conf['database']['charset'];}     

        try {

            switch ($this->db['type']) {
                case 'mysql': 
                    $this->conn = new PDO($this->db['type'].':host='.$this->db['host'].';charset='.$this->db['charset'].';', $this->db['username'], $this->db['password']);
                break;
                case 'sqlsrv':
                    $this->conn = new PDO($this->db['type'].':Server='.$this->db['host'].';', $this->db['username'], $this->db['password']);
                break;
                case 'sqlite':
                    $this->conn = new PDO($this->db['type'].':'.$this->db['dbname']);
                break;
                default:
            ...
            if(in_array($this->db['type'], ['mysql', 'sqlsrv'])){
                ...
                switch ($this->db['type']) {
                    ...
                    case 'sqlite':
                        if(!file_exists($dbname) AND $dbname !== $this->db['dbname']){
                            $this->dbConnect(['database'=>['dbname'=>$dbname]]);
                        }
                    break;
                }
    ...
    public function backup($dbnames, $directory='')
    {
        ...
        $filename = $this->db['type'].'_backup_'.$this->permalink($this->timestamp, array('delimiter'=>'_')).'.json';
        if(!empty($directory)){
            if(is_dir($directory)){
    ...
    public function firewall($conf=array()){
        if(empty($conf['firewall']['allow']['folder'])){
            $conf['firewall']['allow']['folder'] = array('');
        }
        ...
        if($ssl_status === TRUE AND $this->is_ssl() === FALSE){
            $this->abort('400', 'Trang này bắt buộc phải có SSL.');
        }        
        if($hsts_status === TRUE AND ($this->is_ssl() OR $ssl_status == FALSE)){
            $this->abort('503', 'Bắt buộc SSL cho HSTS.');
        }
            ...
            if($_SERVER['REQUEST_METHOD'] === 'POST'){
                if(isset($this->post[$name]) AND isset($_SESSION['csrf']['token'])){
                    if($this->post[$name] == $_SESSION['csrf']['token']){
                        ...
                        $this->abort('401', 'Giá trị của token không được tìm thấy.');
                    }
                } else {
                    $this->abort('400', 'Token không tồn tại.');
                }
        ...
        {
            $this->errors['captcha']['required'] = 'Mã xác nhận không đúng.';
        }
        ...
        if(
            ...
            $this->abort('401', 'Hệ điều hành của bạn không được web vào.');
        }
        ...
        if(
            ...
            $this->abort('401', 'Trình duyệt của bạn không được web vào.');
        }
        ...
        if(
            ...
            $this->abort('401', 'IP của bạn không được web vào..');
        }
        ...
        foreach ($tempLayers as $layer) {
            ...
            if(file_exists($layer['way'].$ext)) { 
                ...
                if(class_exists($className)){ 
                    ...
                    if(!empty($class->monitor['database'])){
                        foreach($class->monitor['database'] as $mondb){
                            $this->monitor['database'][md5($class->sql)] = $mondb;
                        }
                    }
