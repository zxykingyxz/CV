...
class email
{
	...
	public function infoEmail()
	{
		...
		$logo = $this->db->rawQueryOne("select photo from #_bannerqc where hienthi=1 and type=? limit 0,1", array('logo'));
		$social = $this->db->rawQuery("select photo,ten_$lang as ten,link from #_photo where hienthi=1 and type=? order by stt asc,id desc", array("mangxahoi"));
