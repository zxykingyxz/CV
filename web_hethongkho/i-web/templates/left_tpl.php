<nav class="sidebar-nav sidebar-bunker">
    ...
    <ul class="metismenu" id="menu1">
        ...
        <?php if (!empty($GLOBAL['flashsale']) && $config['flash_sale'] == true) {
        ...
        ?>
                <li <?= ($com == 'flashsale' && in_array($_GET['type'], array_keys($GLOBAL['flashsale']))) ? ' class="active"' : '' ?>>
                    ...
                    <a class="has-arrow" href="#" aria-expanded="<?= ($com == 'flashsale' && in_array($_GET['type'], array_keys($GLOBAL['flashsale']))) ? 'true' : 'false' ?>">
                        ...
                        <i class="nav-icon text-sm fal fa-pager"></i>Flash Sale
        ...
        <?php if (count($PRIVATE) > 0) { ?>
            <?php foreach ($PRIVATE as $key => $value) { ?>
                <?php if (!in_array($value, ['gioi-thieu-top'])) { ?>
        ...
        <?php } ?>
        <?php if ($WAREHOUSE) { ?>
            <li class="<?= ($com == 'warehouse') ? "active" : "" ?>">
                <a class="has-arrow" href="#" aria-expanded="<?= ($com == 'warehouse') ? "true" : "false" ?>">
                    <span>
                        <i class="nav-icon text-sm fal fa-boxes"></i>Quản lý hệ thống kho
                    </span>
                </a>
                <ul aria-expanded="<?= ($com == 'warehouse') ? "" : "style='height: 0px;'" ?>">
                    <li class=" <?= ($com == 'warehouse' && $type == 'account') ? "this" : "" ?>">
                        <a href="index.php?com=warehouse&type=account&act=man&page=1">Tài Khoản Thành Viên</a>
                    </li>
                </ul>
            </li>
        <?php } ?>
