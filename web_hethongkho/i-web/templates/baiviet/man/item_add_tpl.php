    ...
    <div class="control_frm">
        <div class="bc">
            <ul id="breadcrumbs" class="breadcrumbs">
                <li><a href="index.html?com=baiviet&act=add<?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?><?php if ($_GET['page'] != '') echo '&page=' . $_GET['page']; ?>"><span><?= $table['title'] ?></span></a> </li>
            ...
            <div class="oneOne">
                <div class="widget mtop0">
                    <div class="title chonngonngu" style="border-bottom: 0px solid transparent">
                        <ul> <?php foreach ($config['lang'] as $k => $v) { ?> <li><a href="<?= $k ?>" class="<?= ($k == 'vi') ? 'active' : '' ?> tipS" title="<?= $v ?>"><img src="./images/<?= $k ?>.png" alt="" class="<?= $func->changeTitle($v) ?>" /><?= $v ?></a> </li> <?php } ?> </ul>
                    </div>
                </div>
            </div>
            <div class="<?= ($table['full'] == true) ? 'oneOne' : 'colLeft' ?>">
                <div class="widget mtop0">
                    <div style="padding:10px;"> <a class="btn btn-sm bg-gradient-primary text-white" href="index.php?com=baiviet&act=man&type=<?= $_GET["type"] ?>&page=<?= $_GET["page"] ?>"> <i class="fas fa-backward mr-2"></i>Trở lại danh sách </a> </div>
                    <div class="title"><img src="./images/icons/dark/record.png" alt="" class="titleIcon" />
                        <h6>Thông tin chung</h6>
                    </div>
                    <?php foreach ($config['lang'] as $k => $v) { ?> <div class="formRow lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>">
                    ...
                    <?php } ?>
                    <?php if ($table['alias'] == true) { ?> <?php foreach ($config['lang'] as $k => $v) { ?> <div class="formRow lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>"> <label class="d-block">[<?= $v ?>] Đường dẫn hiển thị (Bạn có thể thay đổi đường dẫn) :</label>
                                <div class="formRight">
                                    <div class="box-alias" style="position: relative;display: -ms-flexbox;display: flex;-ms-flex-wrap: wrap;flex-wrap: wrap;-ms-flex-align: stretch;align-items: stretch;width: 100%;">
                                        <div class="alias-text-disabled"> <b><?= $https_config ?>
                                                <?php if ($config['alias']) { ?><?= @$_GET['type'] ?>/<?php } ?></b>
                                        </div>
                                        ...
                                        <?php if ($_GET['act'] == 'edit') { ?> <div class="input-group-append" style="display:flex;align-items:center;margin-left:10px">
                                                <div class="input-group-text">
                                                    <input type="checkbox" id="checkUrl<?= $k ?>" data-id="<?= $k ?>" class="change_alias alias_<?= $k ?>" checked> <label style="float:right;padding:0;margin-left:5px;" for="checkUrl<?= $k ?>" class="mb-0 ml-1">Không đổi link</label>
                                                </div>
                                            </div>
                                        <?php } ?>
                                    </div>
            ...
            <div class="<?= ($table['full'] == true) ? 'oneOne' : 'colRight' ?>">
                <div class="widget mtop0">
                    ...
                    <?php if ($table['tacgia'] == true) {
                        ...
                        <div class="formRow">
                            <label>Tác giả</label>
                            <div class="formRight"> <select name="data[id_tacgia]" id="id_tacgia" class="main_select">
                                    <option value="">Chọn tác giả</option> <?php foreach ($row_tacgia as $key => $value) { ?> <option <?php ($item["id_tacgia"] == $value["id"]) ?  "selected" : ''; ?> value="<?= $value["id"] ?>"><?= $value["ten"] ?></option> <?php } ?>
                                </select> </div>
                            <div class="clear"></div>
                        </div> <?php } ?>
                    <?php if ($table['index_robots'] == true) {
                        $robots_array = ["noindex,nofollow", "index,follow"];                    ?>
                        <div class="formRow">
                            <label>Tùy chọn index</label>
                            <div class="formRight"> <select name="data[index_robots]" id="index_robots" class="main_select">
                                    <option value="">Chọn tùy chọn</option> <?php foreach ($robots_array as $key => $value) { ?> <option <?php ($item["index_robots"] == $value) ? "selected" : '' ?> value="<?= $value ?>"><?= $value ?></option> <?php } ?>
                                </select> </div>
                            <div class="clear"></div>
                        </div>
                    <?php } ?>
                    ...
                    <?php if ($table['list'] == true) { ?>
                        <div class="formRow">
                            ...
                            <div class="formRight"> <select class="main_select" onchange="onChangePage(this.value,'baiviet_cat','<?= $_GET['type'] ?>','id_cat','id_list')" name="data[id_list]" id="id_list">
                                    <option value="0">Chọn danh mục cấp 1</option>
                                    <?php if (!empty($items_list)) { ?> <?php for ($i = 0; $i < count($items_list); $i++) { ?> <option value="<?= $items_list[$i]['id'] ?>" <?= ($item['id_list'] == $items_list[$i]['id']) ? 'selected' : '' ?>>
                                                <?= $items_list[$i]['ten_vi'] ?></option> <?php } ?>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="clear"></div>
                    ...
                    <?php if ($table['cat'] == true) { ?>
                        <div class="formRow">
                            ...
                            <div class="formRight"> <select class="main_select" onchange="onChangePage(this.value,'baiviet_item','<?= $_GET['type'] ?>','id_item','id_cat')" name="data[id_cat]" id="id_cat">
                                    <option value="0">Chọn danh mục cấp 2</option>
                                    <?php if (!empty($items_cat)) { ?> <?php for ($i = 0; $i < count($items_cat); $i++) { ?> <option value="<?= $items_cat[$i]['id'] ?>" <?= ($item['id_cat'] == $items_cat[$i]['id']) ? 'selected' : '' ?>>
                                                <?= $items_cat[$i]['ten_vi'] ?></option> <?php } ?>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="clear"></div>
                    ...
                    <?php if ($table['item'] == true) { ?>
                        <div class="formRow">
                            ...
                            <div class="formRight"> <select class="main_select" onchange="onChangePage(this.value,'baiviet_sub','<?= $_GET['type'] ?>','id_sub','id_item')" name="data[id_item]" id="id_item">
                                    <option value="0">Chọn danh mục cấp 3</option>
                                    <?php if (!empty($items_item)) { ?> <?php for ($i = 0; $i < count($items_item); $i++) { ?> <option value="<?= $items_item[$i]['id'] ?>" <?= ($item['id_item'] == $items_item[$i]['id']) ? 'selected' : '' ?>>
                                                <?= $items_item[$i]['ten_vi'] ?></option> <?php } ?>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="clear"></div>
                    ...
                    ?>
                        <div class="formRow">
                            ...
                            <div class="formRight"> <select multiple="multiple" name="data[id_thuonghieu][]" class="thuonghieu"> <?php foreach ($data_thuonghieu as $k => $v) { ?>
                    ...
                    ?>
                        <div class="formRow">
                            ...
                            <div class="formRight"> <select multiple="multiple" name="data[khuyenmai][]" class="select2"> <?php foreach ($data_khuyenmai as $k => $v) { ?>
                                        <option value="<?= $v['id'] ?>" <?= (isset($ex_tags) && count($ex_tags) > 0) ? ((in_array($v['id'], $ex_tags)) ? 'selected' : '') : '' ?>>
                                            <?= $v['ten_vi'] ?></option>
                                    <?php } ?>
                                </select>
                    ...
                    <?php if ($table['tinhtrang'] == true) { ?>
                        <div class="formRow">
                            ...
                            <div class="formRight">
                                <select name="data[status]" id="status" class="main_select">
                                    <option value="1" <?= (@$item['status'] == 1) ? 'selected' : '' ?>>Còn hàng </option>
                                    <option value="2" <?= (@$item['status'] == 2) ? 'selected' : '' ?>>Đã hết hàng </option>
                                </select>
                            </div>
                            <div class="clear"></div>
                        </div>
                    ...
                    <?php if ($table['photo'] == true) { ?>
                        <?php foreach ($table['photo'] as $key_img => $value_img) { ?>
                            <div class="formRow">
                                <label><?= $value_img['title'] ?>:</label>
                                <div class="formRight">
                                    <?php if ($_GET['act'] == 'edit' || $_GET['act'] == 'copy') { ?>
                                        <input type="file" id="<?= $value_img['name'] ?>" name="<?= $value_img['name'] ?>" />
                                    <?php } else { ?>
                                        <input <?= ($value_img['required']) ? 'data-validation="required"' : "" ?> data-validation-allowing="jpg, png" data-validation-max-size="300kb" data-validation-error-msg-required="Bạn chưa chọn file" type="file" id="file" name="file" />
                                    <?php } ?>
                                    <img src="./images/question-button.png" alt="Upload hình" class="icon_question tipS" original-title="Tải hình ảnh (ảnh JPEG, GIF , JPG , PNG)">
                                    <br /> <br />
                                    <span style="display: inline-block; line-height: 30px;color:#CCC;">
                                        Width: <?= $value_img['width'] * $value_img['ratio'] ?>px - Height:<?= $value_img['height'] * $value_img['ratio'] ?>px
                                    </span>
                                </div>
                                <div class="clear"></div>
                            </div>
                            <?php if ($_GET['act'] == 'edit' || $_GET['act'] == 'copy') { ?>
                                <div class="formRow">
                                    ...
                                    <div class="formRight">
                                        <div class="mt10">
                                            <img style="max-width:clamp(100px,100%,250px); height: auto;width: 100%;" src="<?= _upload_baiviet . $item[$value_img['name']] ?>" alt="NO PHOTO" width="<?= $value_img['width'] ?>" height="<?= $value_img['height'] ?>" />
                                        </div>
                                    </div>
                                    <div class="clear"></div>
                                </div>
                                ...
                                <div class="formRight">
                                    <div class="mt10">
                                        ...
                                        <div class="mt10" style="margin-top: 10px;">
                                            <span style="color:blue;display:block;font-size:16px;" class="mt20">
                                                <span style="color:#000;">Tên tệp video: </span> <?= @$item["video"] ?>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="formRow">
                        <div class="formRight"> <label class="stardust-checkbox"> Hiển thị
                ...
                </div> <?php if ($table['seo'] == true) { ?> <?php if (isset($table['seo']) && $table['seo'] == true) {
                                                                    $seoDB = $seo->getSeoDB($id, $com, 'man' . $tbl, $type);                    ?> <div class="widget mtop10">

                            <div class="formRow">
                                <div class="formRight"> <?php foreach ($config['seo-lang'] as $k => $v) { ?>
                                        <div class="box-seo lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>">
                                            <p style="font-size:18px;font-weight:normal;border-bottom:1px solid #ccc;padding-bottom:10px"> Hiển thị kết quả tìm kiếm google search</p>
                                            <p style="font-size:14px;font-weight:normal;padding:10px 0px"> <a href="<?= $https_config ?><?= $item["tenkhongdau_$k"] ?>" target="_blank" title="<?= $https_config ?>"><span><?= $https_config ?></span><span id="url_seo_<?= $k ?>"><?= $item["tenkhongdau_$k"] ?></span></a> </p>
                                            <h3 style="font-size: 20px;line-height: 1.3;color: #1a0dab;margin-bottom: 0px;font-weight:500" class="title_seo" id="title_seo"> <?= ($seoDB['title_' . $k] != '') ? $seoDB['title_' . $k] : '[SEO Onpage] là gì? Hướng dẫn tối ưu SEO Onpage...' ?> </h3>
                                            <p style="padding-top:10px;font-size:14px;line-height: 1.57;word-wrap: break-word;color: #6a6a6a;margin-bottom: 0px;" class="description_seo" id="description_seo"> <?= ($seoDB['description_' . $k] != '') ? $seoDB['description_' . $k] : ' Hướng dẫn SEO onpage căn bản tối ưu để trang web chuẩn SEO lên top Google nhanh và bền vững, kỹ thuật tối ưu SEO onpage đơn giản' ?> </p>
                                        </div> <?php } ?>
                                </div>
                            </div>
                        </div>
                        <div class="widget mtop10">
                            <div class="title"><img src="./images/icons/dark/record.png" alt="" class="titleIcon" />
                                <h6><?= _noidungseo ?></h6>
                            </div> <?php foreach ($config['seo-lang'] as $k => $v) { ?> <div class="formRow lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>">
                                    <label>Title [ <?= $v ?> ]: </label>
                                    <div class="formRight">
                                ...
                                <?php if ($table["schema"] == true) { ?> <div class="formRow lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>">
                                        <label>Schema Json <a href="https://developers.google.com/search/docs/advanced/structured-data/search-gallery" target="_blank">(Tài liệu tham khảo)</a></label> <!-- <button type="submit" name="buildSchema" id="createSchema" class="btn btn-sm bg-gradient-primary text-white" style="margin-bottom:10px;">                                    Tạo cấu trúc                                </button> -->
                                        <div class="formRight">
                                            <textarea rows="8" cols="" id="contentSchema" class="tipS" name="dataseo[schema]"><?= @$seoDB['schema'] ?></textarea>
                                        </div>
                                        <div class="clear"></div>
                                    </div> <?php } ?> <div class="formRow lang_hidden lang_<?= $k ?> <?= ($k == 'vi') ? 'active' : '' ?>">
                                    <div class="formRight">
                        ...
                        </div> <?php } ?> <?php } ?> <div class="clear"></div>
            </div>
        </div>
        <div class="formRow fixedBottom sidebar-bunker">
            <div class="formRight">
                ...
                <div class="box-admin" style="display:flex; align-items:center;justify-content:flex-end">
                    <div class="box-action"> <button type="submit" class="btn btn-sm bg-gradient-primary text-white btn-disabled" <?php if ($_GET["act"] == 'copy' && $table["alias"] == true) echo 'disabled'; ?>> <i class="far fa-save mr-2"></i><?= ($_GET["act"] == 'copy') ? 'Lưu copy' : 'Lưu' ?> </button> <button type="submit" class="btn btn-sm bg-gradient-success btn-disabled" name="save-here" <?php if ($_GET["act"] == 'copy' && $table["alias"] == true) echo 'disabled'; ?>><i class="far fa-save mr-2"></i>Lưu
                            tại trang</button> <button type="reset" class="btn btn-sm bg-gradient-secondary"><i class="fas fa-redo mr-2"></i>Làm lại</button> <a class="btn btn-sm bg-gradient-danger text-white" href="index.html?com=baiviet&act=man<?php if ($_GET['type'] != '') echo '&type=' . $_GET['type']; ?>"> <i class="fas fa-sign-out-alt mr-2"></i>Thoát </a> </div>
                </div>
            </div>
        </div>
        <div class="clear"></div>
                        ...
                        <div class="jFiler-item-thumb">\
                            <div class="jFiler-item-status"></div>\
                                <div class="jFiler-item-info">\
                                    ...
                                    <input type="text" name="stthinh[]" class="stthinh" />\
                                </div>\
                            </div>\
                        </li>',
