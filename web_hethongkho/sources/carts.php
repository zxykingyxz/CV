	...
	if ($src == 'thanh-toan') {
		if (isset($_POST["checkout"])) {
			...
			if (count($cart->checkArrayChecked($_SESSION['cart'])) <= 0) {
				$func->transfer(_khongcosanphamthanhtoan,  $func->getType('carts?src=gio-hang'));
			}
			...
			if ($id_insert) {
				for ($i = 0; $i < $max; $i++) {
					if ($_SESSION['cart'][$i]['checked'] === 1) {
						...
						$proinfo = $cart->getProduct($pid);
						$code = $_SESSION['cart'][$i]['code'];
						$_price = $cart->getPrice($pid, $attribute);
						$old_price = $cart->getPrice($pid, $attribute, true);
			...
			if ($id_insert) {
				/*
					$classMomo = new classMomo();
					$dataMomo = [
						'partnerCode' => 'MOMO1KON20230403',
						'partnerName' => 'test',
						"storeId" => 'DH0001',
						'amount' => $total_price,
						'orderId' => $order_code,
					...
					$classMomo->sendRequestMomo($dataMomo);
					*/
