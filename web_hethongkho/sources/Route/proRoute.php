...
switch ($rowc['type']) {
    case 'photo':
        ...
        break;
    case 'general':
        $tintuc = $cache->getCache("select *,tenkhongdau_$lang as tenkhongdau from #_baiviet where hienthi=1 $where and type=? $order_by limit $startpoint,$per_page", array($type), 'result', _TIMECACHE);
    ...
    case 'cat':
        ...
        $data['breadcrumbs'][0] = array('alias' => $type, 'name' => $title_seo);
        $row_list = $cache->getCache("select id,ten_$lang,tenkhongdau_$lang as tenkhongdau,type from #_baiviet_list where hienthi=1 and id=? and type=? order by stt asc, id desc", array($row_cat['id_list'], $type), 'fetch', _TIMECACHE);
        $data['breadcrumbs'][1] = $func->getArray($row_list);
    ...
    case 'item':
        ...
        $data['breadcrumbs'][0] = array('alias' => $type, 'name' => $title_seo);
        $row_list = $cache->getCache("select id,ten_$lang,tenkhongdau_$lang as tenkhongdau,type from #_baiviet_list where hienthi=1 and id=? and type=? order by stt asc, id desc", array($row_item['id_list'], $type), 'fetch', _TIMECACHE);
        $data['breadcrumbs'][1] = $func->getArray($row_list);
...
// if (!empty($pagingLink) && !empty($method)) {
