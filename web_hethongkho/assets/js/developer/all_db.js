...
$(document).ready(function() {
    // Thêm CSS vào các phần tử có lớp .show_contents_js
    $('body').find('.show_contents_js').each(function() {

        var _this = $(this);
        var numChildren = _this.children().length;

        if (numChildren >= 10) { // Kiểm tra nếu có ít nhất 10 dòng
            if (_this.hasClass('on')) {
                var text_show = 'Thu Gọn Lại';
                var icons_show = '<i class="fas fa-angle-up"></i>';
            } else {
                var text_show = 'Hiển Thị Thêm';
                var icons_show = '<i class="fas fa-angle-down"></i>';
            }
            var newDiv = $('<div class="btn_showhiden_contents"><span>' + text_show + '</span>' + icons_show + '</div>');
            _this.append(newDiv);
        }
    });

    $('body').on('click', '.btn_showhiden_contents', function() {
        var _this = $(this);
        var _form = _this.closest('.show_contents_js');
        var numChildren = _form.children().length;

        if (numChildren >= 10) { // Kiểm tra lại khi bấm nút
            _form.find('div.btn_showhiden_contents').remove();
            if (_form.hasClass('on')) {
