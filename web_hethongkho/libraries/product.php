...
class product
{
    ...
    public function getMans()
    {
        ...
        if ($_GET['keyword'] != '') {
            ...
            $where .= " and " . $this->_func->getSqlWhereKeywords($keyword, ["ten_$lang"]);
        }
        ...
        $total = $count['num'];
        $paging = $this->_func->paginationAdmin($total, $perPage, $page, $url);        // $_SESSION['adm_crp']=$this->_func->getCurrentPageURLAdmin();
    }
    ...
    public function saveMan()
    {
        ...
        foreach ($table['photo'] as $key_img => $value_img) {
            $file = $_FILES[$value_img['name']];
            if (!empty($file)) {
                if ($file['error'] == 0) {
                    $photo = $this->_func->uploadImg((!empty($id)) ? $id : 0, "photo", "thumb", $file, $folder, $this->_com, $$value_img['width'], $$value_img['height'], $$value_img['ratio'], false);
                    $send[$value_img['name']] = $photo[$value_img['name']];
                    $send['thumb_' . $value_img['name']] = $photo['thumb'];
                }
            }
        }
