<label class=" <?= $class_form ?>">
    <?php if ($no_lable !== true) { ?>
        ...
        <div class="mt-1">
        <?php } ?>
        <select name="data[<?= $data ?>]" size="0" id="<?= $id ?>" class=" <?= $class ?> px-3 h-9 bg-white border shadow-sm border-slate-300 placeholder-slate-400 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1  disabled:shadow-none" <?= ($required) ? "required" : "" ?> <?= $function ?>>
