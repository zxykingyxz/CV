...
if ($total_page > 1) {
    ...
    $class_active = " active h-8";
?>
    <div class="flex justify-center items-center content-center gap-1">
        <?php if ($page != 1) { ?>
            <a href="<?= $url . 1 ?>" title="Trang đầu" class="text-xs text-black h-6 aspect-[1/1] rounded-sm border border-gray-100 shadow bg-white hover:bg-blue-600 hover:border-blue-600 hover:text-white [&.active]bg-blue-600 [&.active]:border-blue-600 [&.active]text-white overflow-hidden transition-all duration-300 inline-flex justify-center items-center">
                <i class="fas fa-angle-double-left"></i>
            </a>
            <a href="<?= $url . ($page - 1) ?>" title="Trang trước" class="text-xs text-black h-6 aspect-[1/1] rounded-sm border border-gray-100 shadow bg-white hover:bg-blue-600 hover:border-blue-600 hover:text-white [&.active]bg-blue-600 [&.active]:border-blue-600 [&.active]text-white overflow-hidden transition-all duration-300 inline-flex justify-center items-center">
                <i class="fas fa-angle-left"></i>
            </a>
            <div class="inline-flex justify-center items-center h-6 mx-1">
        ...
        <?php foreach ($list_button as $value_page) { ?>
            <a href="<?= $url . $value_page ?>" title="Trang <?= $value_page ?>" class=" <?= ($value_page == $page) ? $class_active : '' ?> text-xs text-black h-6 aspect-[1/1] rounded-sm border border-gray-100 shadow bg-white hover:bg-blue-600 hover:border-blue-600 hover:text-white [&.active]:bg-blue-600 [&.active]:border-blue-600 [&.active]:text-white overflow-hidden transition-all duration-300 inline-flex justify-center items-center">
                <span>
                    <?= $value_page ?>
                </span>
            </a>
        ...
        <?php if ($page != $total_page) { ?>
            ...
            <a href="<?= $url . ($page + 1) ?>" title="Trang sau" class="text-xs text-black h-6 aspect-[1/1] rounded-sm border border-gray-100 shadow bg-white hover:bg-blue-600 hover:border-blue-600 hover:text-white [&.active]bg-blue-600 [&.active]:border-blue-600 [&.active]text-white overflow-hidden transition-all duration-300 inline-flex justify-center items-center">
                <i class="fas fa-angle-right"></i>
            </a>
            <a href="<?= $url . $total_page ?>" title="Trang cuối" class="text-xs text-black h-6 aspect-[1/1] rounded-sm border border-gray-100 shadow bg-white hover:bg-blue-600 hover:border-blue-600 hover:text-white [&.active]bg-blue-600 [&.active]:border-blue-600 [&.active]text-white overflow-hidden transition-all duration-300 inline-flex justify-center items-center">
