...
class handleTable
{
    ...
    private $array_insert_check_table = [];
    private $array_truncate_table = [];
    private $cacheFile = '/cache/dataTableCache.json';
    ...
    public function getTableList()
    {
        try {
            ...
            // Thực hiện truy vấn lấy danh sách bảng
            $tables = $this->rawQuery("SHOW TABLES;");

            if ($tables) {
                $tableNames = [];
                foreach ($tables as $table) {
                    // Lấy giá trị đầu tiên trong mỗi bảng (tên bảng)
    ...
    // kiểm tra dữ liệu có trong bảng chưa
    public function checkDataInTableSqlConditional($nameTable = null, $array_column = array(), $array_column_check = array(), $array_data = array())
    {
        if (!empty($array_data)) {
            $sql_table = "SELECT EXISTS (SELECT 1 FROM " . $nameTable . " WHERE ";
            $times_check = 0;
            foreach ($array_column as $key => $column) {
                if (!empty($array_data[$key]) && in_array($column, $array_column_check)) {
                    if ($times_check != 0) {
                        $sql_table .= " AND ";
                    }
                    $sql_table .= " $column='" . $array_data[$key] . "' ";
        ...
        if (!empty($this->array_table)) {
            foreach ($this->array_table as $key_table => $value_table) {
                if (!empty($value_table) && !empty($key_table)) {
                    ...
                    var_dump("Có dữ liệu $key_table khai báo không hợp lệ");
                    die;
                }
            }
        }
    ...
    public function autoInsertTable()
    {
        if (!empty($this->array_insert_table)) {
            foreach ($this->array_insert_table as $key_table => $value_insert) {
                if (!$this->checkTableExists($key_table)) {
                    var_dump("Hàm setInsertTableSql Bảng $key_table không tồn tại!");
                    die;
                }

                if (empty($value_insert)) {
                    var_dump("Hàm setInsertTableSql Dữ liệu truyền vào cho bảng $key_table đang rỗng!");
                    die;
                }
                ...
                foreach ($value_insert as $array_data) {
                    if (empty($array_data)) {
                        var_dump("Hàm setInsertTableSql Bảng $key_table có trường dữ liệu bị rỗng!");
                        die;
                    }
                    ...
                    if (!$check_data) {
                        ...
                        $array_data = array_map(function ($value_sql) {
                            return is_null($value_sql) ? 'NULL' : "'" . htmlspecialchars($value_sql) . "'";
                        }, $array_data);
    ...
    public function autoInsertCheckTable()
    {
        if (!empty($this->array_insert_check_table)) {
            foreach ($this->array_insert_check_table as $key_table => $value_table) {
                if (!$this->checkTableExists($key_table)) {
                    var_dump("Hàm setInsertCheckTableSql Bảng $key_table không tồn tại!");
                    die;
                }
                if (empty($value_table['column'])) {
                    var_dump("Hàm setInsertCheckTableSql Cột kiểm tra truyền vào cho bảng $key_table đang rỗng!");
                    die;
                }
                if (empty($value_table['value'])) {
                    var_dump("Hàm setInsertCheckTableSqlDữ liệu truyền vào cho bảng $key_table đang rỗng!");
                    die;
                }

                $value_insert = array_map('unserialize', array_unique(array_map('serialize', $value_table["value"])));
                ...
                foreach ($value_insert as $array_data) {
                    if (empty($array_data)) {
                        var_dump("Hàm setInsertCheckTableSql Bảng $key_table có trường dữ liệu bị rỗng!");
                        die;
                    }
                    $check_data = $this->checkDataInTableSqlConditional($key_table, $array_name_column, $value_table["column"], $array_data);
                    if (!$check_data) {
                        // Kiểm tra kiểu dữ liệu
                        $this->checkTypeColumnTable($key_table, $array_type_column, $array_data);
    ...
    // Thêm Dữ liệu vào bảng
    // check tất cả 
    public function setInsertTableSql($nameTable = null, $arrayValue = array())
    {
        $this->array_insert_table[$nameTable] = $arrayValue;
    }
    // check có điều kiện
    public function setInsertCheckTableSql($nameTable = null, $arrayColumn = array(), $arrayValue = array())
    {
        $this->array_insert_check_table[$nameTable]["column"] = $arrayColumn;
        $this->array_insert_check_table[$nameTable]["value"] = $arrayValue;
    }
    ...
    public function autoHandleTable()
    {
        ...
        $data_handle = [];
        $data_handle["create_table"] = $this->array_table;
        $data_handle["insert_table"] = $this->array_insert_table;
        $data_handle["insert_check_table"] = $this->array_insert_check_table;
        $data_handle["truncate_table"] = $this->array_truncate_table;

        $this->connect();
        $data_table_check = $this->getTableList();

        $check_handle = false;
        if (empty($data_check) || ($data_check["create_table"] != $data_handle["create_table"]) || empty($data_table_check) || (sort($data_table_check) != sort(array_keys($this->array_table)))) {
            $this->autoCreateTable();
            $check_handle = true;
        }
        if (empty($data_check) || ($data_check["insert_table"] != $data_handle["insert_table"])) {
            $this->autoInsertTable();
            $check_handle = true;
        }
        if (empty($data_check) || ($data_check["insert_check_table"] != $data_handle["insert_check_table"])) {
            $this->autoInsertCheckTable();
            $check_handle = true;
        }
        if (empty($data_check) || ($data_check["truncate_table"] != $data_handle["truncate_table"])) {
