...
$class_form_seclect = "w-full rounded ";
...
<?php if ($form !== false) { ?>
    <label class=" <?= $class_form ?>">
        <?php if ($no_label !== true) { ?>
            ...
            <?php } ?>
        <?php } ?>
        <div class="bg-white border shadow-sm border-inherit placeholder-slate-400 has-[:disabled]:bg-slate-50 has-[:disabled]:text-slate-500 has-[:disabled]:border-slate-200 has-[:focus]:outline-none has-[:focus]:border-border-blue-300 has-[:focus]:ring-border-blue-300 block  has-[:focus]:ring-1  has-[:disabled]:shadow-none <?= $class_form_seclect ?>">
