FRAMEWORK = {
    ...
    detailWeb: function () {
        // Toàn màng hình
        $("body").on("click", ".button_delete_items_congno", function () {
            $(this).closest(".form_items_congno").remove();
        });
        $("body").on("click", ".button_add_items_congno", function () {
            $.ajax({
                url: "ajax/layouts/items_congno.php",
                type: "POST",
                data: "",
                dataType: "JSON",
                beforeSend: function () {
                ...
                success: function (response) {
                    if (response.data.status == 200) {
                        $("body .form_all_items").append(response.html);
                        FRAMEWORK.iconsWeb();
                        FRAMEWORK.formInputAdmin();
                    } else {
    ...
    handleTable: function () {
        ...
        $("body").on("click", ".button_delete_all_default", function () {
            let _this = $(this);
            let url = _this.data("url");
            let list_delete = $('input[name="check_box_default"]:checked')
                .map(function () {
                    return this.value;
                })
                .get()
