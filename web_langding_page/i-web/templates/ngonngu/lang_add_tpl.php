...
<div class="wrapper">
	...
	<form name="supplier" class="card-article form" id="validate" novalidate action="index.html?com=ngonngu&act=save_lang<?php if($_REQUEST['currentPage']!='') echo'&currentPage='. $_REQUEST['currentPage'];?>" method="post" enctype="multipart/form-data">
		<div class="mtop0">
			<div class="oneOne">
			...
			<div class="<?=($config_pr['full']==true) ? 'oneOne':'colLeft' ?>">
				<div class="widget mtop0">
					...
					<div class="formRow">
						...
						<div class="formRight">
			                <input required type="text" name="item" title="Nhập tên ngôn ngữ" id="lang_<?=$k?>" class="tipS validate[required]" value="<?=@$item['item']?>" />
						</div>
						<div class="clear"></div>
					</div>
					<?php foreach ($config['lang'] as $k => $v){ ?>
			        <div class="formRow lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
						...
						<div class="formRight">
			                <input required type="text" name="lang_<?=$k?>" title="Nhập tên ngôn ngữ" id="lang_<?=$k?>" class="tipS validate[required]" value="<?=@$item['lang_'.$k]?>" />
						</div>
						<div class="clear"></div>
					</div>
					<?php } ?>
                ...
                item: '<li class="jFiler-item">\
                            <div class="jFiler-item-container">\
                                <div class="jFiler-item-inner">\
                                    ...
                                    </div>\<input required type="text" name="stthinh[]" class="stthinh" />\
                                </div>\
                            </div>\
                        </li>',
