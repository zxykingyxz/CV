    ...
    <div class="z-40 right-2 rounded-md shadow-md border border-gray-200 top-1/2 -translate-y-1/2 w-auto fixed hidden lg:block ">
        <ul class=" bg-white py-2 px-2 rounded-md inline-flex gap-2 flex-col ">
            <?php if ($config['cart']['turn_on'] != false) { ?>
                <li class="relative last:mb-0 group rounded-[4px] border border-gray-100 w-[36px] aspect-[1/1]  transition-all duration-300 flex flex-col  items-center justify-center">
            ...
            <?php if (!empty($tool)) { ?>
                <?php foreach ($tool as $key => $value) { ?>
                    <li class=" group rounded overflow-hidden  transition-all duration-300 flex flex-col  items-center justify-center">
                        <div class=" overflow-hidden flex justify-center items-end w-[36px] aspect-[1/1] rounded shadow border border-gray-100 p-[3px] bg-white hover:bg-gray-100 transition-all duration-300">
                            <?= $func->addHrefImg([
                                'classfix' => '',
                                'class' => '',
                                'addhref' => true,
    ...
    <div class="form_tool_mobile group/toolMobile left fixed bottom-[80px] [&.left]:left-2 [&.right]:right-2 z-40 block lg:hidden">
        ...
        <div class="relative">
            <div class="group/buttonMobile cursor-pointer w-[60px] aspect-[1/1] overflow-hidden rounded-full bg-[var(--html-bg-website)] flex justify-center items-center p-[2px] btn_tool_mobile" data-nb="tool_mobile">
                <div class="w-full leading-none overflow-hidden aspect-[1/1] group-[&.on]/buttonMobile:hidden">
                    <img src="assets/images/icon/icons_tool.png" alt="Hiện" class="overflow-hidden w-full rounded-full aspect-[1/1]">
                </div>
                <div class="hidden group-[&.on]/buttonMobile:block ">
                    <i class="fas fa-times text-[35px] text-white"></i>
                </div>
            </div>
            <div class="absolute bottom-[calc(100%+20px)] group-[&.left]/toolMobile:left-0 group-[&.right]/toolMobile:right-0 w-[200px] z-10 hidden data_tool_mobile" data-nb="tool_mobile">
                <div class="w-full grid grid-cols-1 gap-1">
                    <?php if ($config['cart']['turn_on'] != false) { ?>
                        <div class="rounded-md border-2 border-[var(--html-bg-website)] flex items-center justify-center bg-white">
                            <div class="flex-initial w-[50px] aspect-[1/1] flex justify-center items-center p-1 border-r-2 border-[var(--html-bg-website)]">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" enable-background="new 0 0 512.003 512.003" height="32" viewBox="0 0 512.003 512.003" width="32">
                                    <g>
                                        <path d="m86.1 91.002 17.017 15 14.206-15-14.033-49.117c-1.846-6.445-7.734-10.884-14.429-10.884h-73.861c-8.291 0-15 6.709-15 15s6.709 15 15 15h62.553z" fill="#d5e8fe" />
                                        <path d="m376 481.002c-24.814 0-45-20.186-45-45s20.186-45 45-45 45 20.186 45 45-20.186 45-45 45z" fill="#29376d" />
                            ...
                            <div class="flex-1 px-2 text-base font-medium font-main-500">
...
<div class="sticky bottom-0 py-1 bg-[var(--html-bg-website)] text-white text-xs sm:text-sm w-full z-40 shadow shadow-[var(--html-bg-website)]  block lg:hidden border-t border-gray-200">
    <ul class="flex content-end items-end">
        ...
        <li class="flex-1">
            <a id="chatzalo" class="flex justify-center items-center flex-col " title="Chat zalo" aria-label="Chat zalo" href="https://zalo.me/<?= $func->handlePhoneNumberUrl($row_setting["sozalo"]) ?>">
                <div>
        ...
        <?php if ($config['cart']['turn_on'] == false) { ?>
            <li class="flex-1">
                <a id="nhantin" class="flex justify-center items-center flex-col " title="Nhắn tin" aria-label="Nhắn tin" href="sms:<?= $func->handlePhoneNumberUrl($row_setting["hotline"]) ?>">
                    <div>
        ...
        <?php if ($config['cart']['turn_on'] == true) { ?>
            <li class="flex-1">
                <a href="<?= $func->getType('carts') . '?src=thanh-toan' ?>" title="Giỏ hàng" aria-label="Giỏ hàng" class="mt-[11px] relative flex flex-col items-center">
                    <div class="relative">
...
<?php if (!empty($hotline)) { ?>
    <div class="form_hotline group/hotline left fixed z-40 top-[40%] [&.left]:left-2 [&.right]:right-2 hidden sm:block">
        <div class="relative ">
            <div class="relative cursor-pointer z-10 bg-[var(--html-bg-website)] w-[52px] aspect-[1/1] gap-1 rounded-full flex flex-wrap justify-center items-center content-center text-center text-white btn_hotline" data-nb="hotline_action">
                <div class="leading-none">
                    <i class="fab fa-whatsapp text-[20px] shake_design"></i>
                </div>
                ...
                <div class="absolute z-[-1] top-0 left-0 w-full h-full  rounded-full bg-[var(--html-bg-website)] animate-ping opacity-30"></div>
                <div class="absolute z-[-1] top-0 left-0 w-full h-full  rounded-full bg-[var(--html-bg-website)] animate-ping opacity-50" style="animation-delay: 300ms;"></div>
            </div>
            <div class="absolute top-0 w-[200px] group-[&.left]/hotline:left-[calc(100%+15px)] group-[&.right]/hotline:right-[calc(100%+15px)] rounded-md border-2 border-[var(--html-bg-website)] bg-white px-2 py-3  hidden data_hotline " data-nb="hotline_action">
                <div class="w-full max-h-[200px] overflow-x-hidden overflow-y-auto scroll-design-one">
                    <div class="w-full grid grid-cols-1 gap-2">
                        <?php foreach ($hotline as $key => $value) { ?>
                            <div class="w-full ">
                                <div class="mb-1 capitalize"><?= $value['ten'] ?></div>
                                <div class="line">
                                    <a href="tel:<?= $func->handlePhoneNumberUrl($value['phone']); ?>" title="<?= $value['phone'] ?>" class="text-[var(--html-bg-website)]">
                                        <span class="text-[20px]">
                                            <i class="fas fa-microphone"></i>
                                        </span>
                                        <span class="text-[18px] font-main-600 font-semibold">
                                            <?= $value['phone'] ?>
                                        </span>
                                    </a>
                                </div>
